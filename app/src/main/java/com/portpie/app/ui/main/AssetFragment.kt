...
import com.portpie.app.databinding.FragmentAssetBinding
...
//    private lateinit var adapter: AssetListAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.asset.collect {assetList->
                ...
                assetAdapter.setStocks(assetList)
            }
        }
    ...
    private fun updateChart(assetList: List<OwnedAsset>) {
        ...
        val crypto = assetList
            .filter { it.type == StockType.CRYPTO }
            .sumOf { it.amount * it.price }
        val cash = assetList
            .filter { it.type == StockType.CASH }
            .sumOf { it.amount * it.price }
        val gold = assetList.filter { it.type == StockType.GOLD }
            .sumOf { it.amount*it.price }

        // 🔥 PieChart 항목 3개
        val entries = mutableListOf<PieEntry>()
        if (domestic > 0) entries.add(PieEntry(domestic.toFloat(), "국내"))
        if (foreign > 0)  entries.add(PieEntry(foreign.toFloat(), "해외"))
        ...
        setupPieChart()
        // 총 자산 표시 (원화)
        val total = domestic + foreign + crypto +cash+gold
        binding.tvTotalPrice.text = "총 자산 : ₩%,.0f".format(total)
    }
